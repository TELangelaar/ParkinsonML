...
g = sns.barplot([missing_data.index[0], missing_data.index[4]],
                [missing_data.BirthYear, missing_data.DiagnosisYear],
                palette='bwr_r')
...
# Impact
g = sns.countplot(x='Impact', hue='Parkinsons',
                  order=['None', 'Mild', 'Medium', 'Severe'],
                  data=user_df, palette='bwr_r')
...
all_cols = (['BirthYear', 'Male', 'Parkinsons', 'Tremors', 'DiagnosisYear',
             'Sided_Left', 'Sided_None', 'Sided_Right', 'UPDRS_1', 'UPDRS_2',
             'UPDRS_3', 'UPDRS_4', "UPDRS_Don't know", 'Impact',
             'Levadopa', 'DA', 'MAOB', 'Other'] + column_names)
combined_user_df_copy = combined_user_df.melt(id_vars=all_cols,
                                              value_vars=['Hold time',
                                                          'Latency time',
                                                          'Flight time'],
                                              var_name='Kind',
                                              value_name='Time')
...
sns.boxplot(x='Kind', y='Time', hue='Impact',
            hue_order=order, data=combined_user_df_copy)
plt.title('Mean times for Parkinsons')
plt.ylabel('Time (ms)')
...
sns.scatterplot(x='Hold time', y='Latency time',
                hue='Impact', hue_order=order, data=combined_user_df)
...
sns.scatterplot(x='Hold time', y='Flight time',
                hue='Impact', hue_order=order, data=combined_user_df)
...
sns.scatterplot(x='Flight time', y='Latency time', hue='Impact',
                hue_order=order, data=combined_user_df)
plt.ylabel('Latency time (ms)')
...
# %% Machine Learning
target = 'Parkinsons'
features = ['Male', 'Tremors', 'Sided_Left', 'Sided_Right', 'Sided_None',
            'UPDRS_1', 'UPDRS_2', 'UPDRS_3', 'UPDRS_4', "UPDRS_Don't know",
            'Impact', 'Hold time', 'Latency time', 'Flight time'] + column_names
            
X = combined_user_df[features]
y = combined_user_df[target]

## One Hot Encoder
#features_OHE = ['Male', 'Tremors', 'Sided_Left', 'Sided_Right', 'Sided_None',
#                'UPDRS_1', 'UPDRS_2', 'UPDRS_3', 'UPDRS_4']
#OHE = OneHotEncoder()
#X[features_OHE] = OHE.fit(X[features_OHE])
#
# Label Encoder
features_LE = ['Impact']
LE = LabelEncoder()
X[features_LE] = LE.fit(X[features_LE]).transform(X[features_LE])